...
/// x86-64 ADX tier: the rolled `helios_sos_x86` leaf behind the public SoS
...
mod leaf_differential {
    ...
    fn compare_all(f: &[Fp; 16], case: usize) {
        ...
        // production dispatch links it (HELIOS_SOSD2_ASM).
        assert_eq!(
...
/// path bit for bit, whatever HELIOS_FP6_ASM selects for production
...
/// sosd6 path bit for bit, whatever HELIOS_FP12_034_ASM selects for
...
/// whatever HELIOS_FP12_SQR_ASM selects for production dispatch -- the leaf
...
/// bit for bit, whatever HELIOS_FP12_MUL_ASM selects for production
...
/// whatever HELIOS_CYC_SQR_ASM selects for production dispatch -- the leaf
...
#[cfg(all(test, feature = "std", helios_mont4_x86_64_adx))]
mod sosd2_micro {
