...
pub fn mixed_batch_verify(
    ...
    if has_groth16 {
        groth16::validate_batch_shape(batch.groth16_vks, batch.groth16_proofs)?;
    }
    ...
    let mut pairs: Vec<PodG1G2Pair> = if has_groth16 {
        groth16::fold_pairs(batch.groth16_vks, batch.groth16_proofs, groth16_randomizers)?
    } else {
        Vec::new()
    };
...
fn fold_plonk_tail(
    ...
    for (first, members) in &clusters {
        ...
        let cluster_randomizers: Vec<Fr> = members
            ...
            .collect();
        let (p, negated_q) = plonk::fold_msms(&fold_groups, &cluster_randomizers)?;
