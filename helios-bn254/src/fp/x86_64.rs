...
//! `HELIOS_DUMP_ASM=<absolute dir> cargo build`. This module exists only
//! when `build.rs` emitted `helios_mont4_x86_64_adx`, i.e. the target is
...
unsafe extern "C" {
    fn helios_mont4_mul_x86(
        z: *mut u64,
        x: *const u64,
        y: *const u64,
        constants: *const Mont4Constants,
    );
    fn helios_mont4_sqr_x86(
        z: *mut u64,
        x: *const u64,
        y: *const u64,
        constants: *const Mont4Constants,
...
/// dispatches (twelve `helios_sos_x86` calls with their call/prologue tax
...
/// (six `helios_sos_x86` calls with their call/prologue tax and 72
...
/// `helios_sos_x86` calls lose (measured -13% on the g2 subgroup) is kept.
///
/// The rolled `helios_sosd2_small_x86` leaf implements the contract (its
...
/// Composed dual-lane T = 6 sum through two rolled `helios_sos_x86` walks;
...
/// Two serial `helios_sos_x86` walks are each one ~700-instruction
