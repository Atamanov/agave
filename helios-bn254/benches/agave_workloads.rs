//! Helios timings over the exact deterministic fixtures used by Agave.
//!
//! Every row restarts `StdRng` with Agave's seed and performs the same direct
//! arkworks draws in the same order. Conversion to Helios's canonical byte or
...
/// equivalent Helios type once, outside the timed region.
...
fn fixture_selected(group: &str, n: usize) -> bool {
    let Ok(filter) = std::env::var("HELIOS_BENCH_FILTER") else {
        return true;
    };
    let id = format!("{group}/helios/{n}");
    if filter.contains("/helios/") {
...
fn bench_g1_msm(c: &mut Criterion) {
    ...
    for &n in MSM_SIZES {
        ...
        group.bench_with_input(BenchmarkId::new("helios", n), &n, |bencher, _| {
            bencher.iter(|| {
...
fn bench_g2_subgroup_check(c: &mut Criterion) {
    ...
    if let Ok(filter) = std::env::var("HELIOS_BENCH_FILTER")
    ...
    configure_group(&mut group);
    let mut cursor = 0_usize;
    group.bench_function("helios/point", |bencher| {
        bencher.iter(|| {
