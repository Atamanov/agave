...
fn notation(column: ColumnId, trace: &OperationTrace) -> String {
    ...
    for call in &trace.plonk_multi_vk_reduce_calls {
        ...
        let shape = format!(
            "RED({}c/{}p/{}i)",
            call.contexts, call.proofs, call.public_inputs
        );
        parts.push(if call.calls == 1 {
            shape
        } else {
            format!("{}\u{d7}{shape}", call.calls)
        });
    }
    ...
    if !lincombs.is_empty() {
        ...
        while index < lincombs.len() {
            ...
            let count = lincombs[index..]
                .iter()
                .take_while(|t| **t == terms)
                .count();
            runs.push(if count == 1 {
                format!("LC({terms}t)")
            } else {
    ...
    for row in &measured.rows {
        assert_eq!(
            row.statement_compression_calls
                .saturating_add(row.state_machine_calls),
            row.calls,
