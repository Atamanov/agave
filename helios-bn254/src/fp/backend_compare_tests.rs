...
const SCHEMA: &str = "helios-bn254-backend-compare-v1";
...
fn build_pools(size: usize) -> Pools {
    assert!(
        ...
        "HELIOS_BACKEND_POOL_SIZE must be a power of two"
    );
    assert!(
        size <= 1 << 20,
        "HELIOS_BACKEND_POOL_SIZE is unreasonably large"
    );
...
fn paired_backend_compare() {
    ...
    let samples = configured_usize("HELIOS_BACKEND_SAMPLES", DEFAULT_SAMPLES, 30);
    let iterations = configured_usize("HELIOS_BACKEND_ITERATIONS", DEFAULT_ITERATIONS, 10_000);
    let warmup_iterations = configured_usize(
        "HELIOS_BACKEND_WARMUP_ITERATIONS",
        DEFAULT_WARMUP_ITERATIONS,
        1,
    );
    let pool_size = configured_usize("HELIOS_BACKEND_POOL_SIZE", DEFAULT_POOL_SIZE, 256);
    let output_path = std::env::var_os("HELIOS_BACKEND_OUTPUT")
        .expect("HELIOS_BACKEND_OUTPUT must name a fresh JSONL file");
    ...
    let output = OpenOptions::new()
        ...
        .create_new(true)
        .open(&output_path)
        .expect("HELIOS_BACKEND_OUTPUT must be a creatable, nonexistent file");
