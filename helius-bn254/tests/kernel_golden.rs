...
//! inside OUT_DIR.
//!
//! Blessing is deliberate: `HELIUS_BLESS=1 cargo test --test kernel_golden`
...
fn check(name: &str, rendered: &str) {
    ...
    match std::env::var("HELIUS_BLESS").ok().as_deref() {
        Some("1") => {
        ...
        None => {}
        Some(other) => panic!("HELIUS_BLESS={other:?} is not recognized; use 1 to bless"),
    }
    let expected = fs::read_to_string(&golden).unwrap_or_else(|e| {
        panic!(
            "read {}: {e}\n\
             Missing snapshot? Generate it with:\n  \
             HELIUS_BLESS=1 cargo test --release --features std --test kernel_golden",
            golden.display(),
        )
    });
    if rendered != expected {
        let actual = std::env::temp_dir().join(format!("helius-{name}.actual"));
