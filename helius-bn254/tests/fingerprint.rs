...
fn taxonomy(battery: &mut Battery) {
    let caps = [
        helius_bn254::MSM_MAX_POINTS as u64,
        helius_bn254::PAIRING_MAX_PAIRS as u64,
        helius_bn254::FR_MAX_ELEMS as u64,
        helius_bn254::G1_BYTES as u64,
        helius_bn254::G2_BYTES as u64,
        helius_bn254::PAIR_BYTES as u64,
        helius_bn254::SCALAR_BYTES as u64,
    ];
    battery.absorb(
        "caps",
...
fn msm_battery(battery: &mut Battery) {
    ...
    // still run) and cap + 1.
    let cap = helius_bn254::MSM_MAX_POINTS;
    battery.msm(
        "msm_cap",
        &vec![infinity; cap],
...
fn pairing_battery(battery: &mut Battery) {
    ...
    // cap + 1.
    let cap = helius_bn254::PAIRING_MAX_PAIRS;
...
fn fr_battery(battery: &mut Battery) {
    ...
    // Cap-exact and cap + 1.
    let cap = helius_bn254::FR_MAX_ELEMS;
