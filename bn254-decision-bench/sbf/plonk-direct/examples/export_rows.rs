...
use bn254_decision_plonk_direct_guest::{
    CanonicalPlonkSourceInput, export_canonical_plonk_rows, input_keyset_digest,
    registry_keyset_digest,
};
use serde_json::json;
...
fn main() {
    ...
    while let Some(flag) = arguments.next() {
        match flag.to_str() {
            ...
            _ => panic!("usage: export_rows --fixtures-root ABS --output ABS"),
        }
    }
    ...
    let loaded = ["mul1", "mul2", "mul3"].map(|name| {
        (
            name,
