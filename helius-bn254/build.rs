...
//! `tests/golden/` (reviewed, regenerated with `HELIUS_BLESS=1`, diffed by
...
/// One HELIUS_*_ASM leaf toggle: env override, emitted activation cfg,
...
/// plain two-arm `#[cfg(helius_*_active)]` / `#[cfg(not(...))]`. The
...
const LEAF_TOGGLES: &[Toggle] = &[
    ...
    // HELIUS_SOSD2_ASM=1 opts a build into the asm leaf (AMD deployments).
    Toggle {
        env: "HELIUS_SOSD2_ASM",
        cfg: "helius_sosd2_active",
    ...
    // route's two serial helius_sos_x86 walks (whose single-lane carry
    ...
    // on for AMD, off elsewhere; HELIUS_SOSD6_ASM overrides.
    Toggle {
        env: "HELIUS_SOSD6_ASM",
        cfg: "helius_sosd6_active",
    ...
    // (six helius_sos_x86 calls, 72 pointer-table stores, nine negp temps, or
    ...
    // HELIUS_FP6_ASM=0 restores the composed path.
    Toggle {
        env: "HELIUS_FP6_ASM",
        cfg: "helius_fp6_active",
    ...
    // one call replaces six sosd6 dispatches (twelve helius_sos_x86 calls,
    ...
    // unknown; HELIUS_FP12_034_ASM overrides either way.
    Toggle {
        env: "HELIUS_FP12_034_ASM",
    ...
    // HELIUS_FP12_SQR_ASM overrides.
    Toggle {
        env: "HELIUS_FP12_SQR_ASM",
    ...
    // HELIUS_FP12_MUL_ASM overrides.
    Toggle {
        env: "HELIUS_FP12_MUL_ASM",
    ...
    // defaults on; HELIUS_CYC_SQR_ASM=0 restores the composed path.
    Toggle {
        env: "HELIUS_CYC_SQR_ASM",
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    if let Some(dump) = std::env::var_os("HELIUS_DUMP_ASM") {
        ...
        if !dump.is_absolute() {
            return Err("HELIUS_DUMP_ASM must be an absolute directory path".into());
        }
    ...
    {
        return Err(format!(
            "HELIUS_AVX512_IFMA={other:?} is not recognized. Use 1 to force or 0 to deny"
        )
        .into());
    }
    let ifma_available = arch == "x86_64" && has("avx512f") && has("avx512ifma");
    match (deny_ifma, force_ifma, ifma_env.as_deref()) {
        (true, _, Some("1")) => return Err("deny-ifma rejects HELIUS_AVX512_IFMA=1".into()),
        (true, _, _) => {}
        (_, true, Some("0")) => return Err("force-ifma rejects HELIUS_AVX512_IFMA=0".into()),
        (_, true, _) if !ifma_available => {
            return Err("force-ifma requires avx512f and avx512ifma target features".into());
        }
        (_, true, _) => println!("cargo:rustc-cfg=helius_avx512_ifma"),
        (_, _, Some("1")) if !ifma_available => {
            return Err(
                "HELIUS_AVX512_IFMA=1 requires avx512f and avx512ifma target features".into(),
            );
        }
        (_, _, Some("0")) => {}
        (_, _, _) if ifma_available => println!("cargo:rustc-cfg=helius_avx512_ifma"),
        _ => {}
    }
    Ok(())
}
