...
loops run eight lanes wide on AVX-512 IFMA, helios against mcl, on three x64
server microarchs. Single-thread, taskset-pinned, native flags; helios and mcl
...
# helios: 8-wide IFMA batch. mcl: scalar (it ships no BN254 IFMA).
ROWS = [
    ("Intel Granite Rapids\nXeon 6767P", 137.0, 311.0),
    ("AMD Zen 4\nEPYC 9354", 178.5, 412.6),
    ("Intel Ice Lake\nXeon 8352V*", 353.8, 617.4),
]
FOOTNOTE = "* Ice Lake measured before the squaring-aware tower; understates helios."
HELIOS, MCL = "#2f6fb0", "#e08a2c"

labels = [r[0] for r in ROWS]
helios = np.array([r[1] for r in ROWS])
mcl = np.array([r[2] for r in ROWS])
x = np.arange(len(ROWS))
w = 0.34

fig, ax = plt.subplots(figsize=(9.2, 4.4), dpi=200)
b1 = ax.bar(x - w / 2, helios, w, label="helios (8-wide IFMA)", color=HELIOS)
b2 = ax.bar(x + w / 2, mcl, w, label="mcl", color=MCL)

for i in range(len(ROWS)):
    ax.text(x[i] - w / 2, helios[i] + 8, f"{helios[i]:.0f}", ha="center", va="bottom",
            fontsize=10, color="#333")
    ax.text(x[i] + w / 2, mcl[i] + 8, f"{mcl[i]:.0f}", ha="center", va="bottom",
            fontsize=10, color="#333")
    ax.annotate(f"{mcl[i] / helios[i]:.2f}x faster", (x[i] - w / 2, helios[i] / 2),
...
ax.set_title("BN254 pairing, 8-wide IFMA batch: helios vs mcl", fontsize=13, weight="bold",
             pad=24)
