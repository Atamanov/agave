...
environment toggle values stop the build. `HELIOS_DUMP_ASM` must name an
...
```text
cargo test -p helios-bn254 --features std,deny-ifma
cargo clippy -p helios-bn254 --all-targets --features std,deny-ifma -- -D warnings
cargo clippy -p helios-bn254 --all-targets --features std,force-portable,deny-ifma -- -D warnings
```
